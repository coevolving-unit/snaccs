...
find_fastq_files() {
    ...
    find $fastq_base_dir -name "*${sample_id}*R1*.fastq.gz" | sort > mapped_reads_F/"${sample}"/"${sample}"_R1.txt
    find $fastq_base_dir -name "*${sample_id}*R2*.fastq.gz" | sort > mapped_reads_F/"${sample}"/"${sample}"_R2.txt
    ...
    #     find ${fastq_base_dir}/${batch} -name "*${sample_id}*R1*.fastq.gz" | sort > mapped_reads_F/"${sample}"/"${sample}"_R1.txt
    #     find ${fastq_base_dir}/${batch} -name "*${sample_id}*R2*.fastq.gz" | sort > mapped_reads_F/"${sample}"/"${sample}"_R2.txt
    # fi
}
...
r1_files=$(cat mapped_reads_F/"${sample}"/"${sample}"_R1.txt | tr '\n' ' ')
r2_files=$(cat mapped_reads_F/"${sample}"/"${sample}"_R2.txt | tr '\n' ' ')
